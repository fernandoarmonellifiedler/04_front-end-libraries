...
class LifecycleDemo extends React.Component {
  ...
  //
  // This (and PureComponent) are the primary ways to optimize class components. If you notice performance is slow, measure with the profiler, then try implementing this method to prevent needless renders. React is fast out of the box, and a few extra renders won't hurt. I wouldn't recommend implementing this method unless you know you need it.
  shouldComponentUpdate(nextProps, nextState) {
    console.log('[shouldComponentUpdate]', 'Deciding to update');
    return true;
  }
  // Called after render() but before updating the DOM
  // A good time to make calculations based on old DOM nodes. The value returned here is passed into componentDidUpdate
  getSnapshotBeforeUpdate(nextProps, nextState) {
    console.log('[getSnapshotBeforeUpdate]', 'About to update...');
    return `Time is ${Date.now()}`;
  }
  // Called after render() and after updating the DOM. The whole render/commit/update cycle is done.
  // This is a good time to check if a prop has changed, by checking prevProps.whatever === this.props.whatever. Useful for re-fetching data when a record ID changes.
